 ...
 */
export async function getWorkspace(tree: Tree, path = DEFAULT_WORKSPACE_PATH): Promise<WorkspaceDefinition> {
 ...
 * will be used.
 */
export async function writeWorkspace(tree: Tree, workspace: WorkspaceDefinition, path?: string): Promise<void> {
  const host = new TreeWorkspaceHost(tree);
